...
class Homepage(Page):
   def __init__(self, *args, **kwargs):
       ...
       add = tk.Button(lframe, text = "  +  ", command = None,width=7)           #word pop-up ipv framelayer
       remove = tk.Button(lframe, text = "  -  ", command = lambda: root.show_frame("RemoveRoom"),width=7)     #word pop-up ipv framelayer
...
class MainView(tk.Frame):
    def __init__(self, *args, **kwargs):
        tk.Frame.__init__(self, *args, **kwargs)
        homepage = Homepage(self) #call homepage
        addroom = AddRoom(self)    #Add a room page

        container = tk.Frame(self)
        container.pack(side="top", fill="both", expand=True)

        homepage.place(in_=container, x=0, y=0, relwidth=1, relheight=1)
        addroom.place(in_=container, x=0, y=0, relwidth=1, relheight=1)
        #als je hier een button maakt kan ik wel switchen tussen de pages.

        homepage.show()       #show homepage first
